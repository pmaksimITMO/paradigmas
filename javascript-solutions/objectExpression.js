...
let Meansq = createOperation(
    ...
    (varName, ...args) => new Divide(
        new Multiply(new Const(2), args.reduce((prev, cur) => new Add(prev, new Multiply(cur.diff(varName), cur)), new Const(0))),
...
let parseVariant = (str, isPrefix) => {
    ...
    let ans = elements.reduce(
        function (stack, element, position) {
            ...
            if (position + 1 < elements.length && !((elements[position + 1] === closeBracket) ^ !(element in operations))) {
                throw new OperationMismatch(element, "operation", position + 1, str);
            } else if (element === openBracket) {
                balance++;
                countArgs.push(curArgs);
                curArgs = 0;
            } else if (element === closeBracket) {
                balance--;
                curArgs += countArgs.length === 0 ? 0 : countArgs.pop();
            } else {
                if (Variable.varNames.has(element)) {
                    ...
                    if (isNaN(+element)) {
                        throw new IllegalArgument(element, "number/operation with space", position, str);
                    }
                    curArgs++;
